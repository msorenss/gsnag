...
def main():
    for tool in ("dpkg-deb", "dpkg-shlibdeps", "dpkg", "strip", "desktop-file-validate"):
        if not shutil.which(tool):
            raise SystemExit(f"Missing {tool}; install dpkg-dev, binutils and desktop-file-utils")
    ...
    with tempfile.TemporaryDirectory(prefix="gsnag-deb-") as tmp:
        ...
        desktop = install("packaging/gsnag.desktop", "usr/share/applications/gsnag.desktop")
        run("desktop-file-validate", str(desktop))
        install("packaging/gsnag.svg", "usr/share/icons/hicolor/scalable/apps/gsnag.svg")
        ...
        dependencies = run("dpkg-shlibdeps", "-O", str(binary.relative_to(work)), cwd=work)
        depends = next(line.split("=", 1)[1] for line in dependencies.splitlines()
                       if line.startswith("shlibs:Depends="))
        installed_kib = sum(p.stat().st_size for p in stage.rglob("*") if p.is_file())
